...
namespace Công_nghệ_phần_mềm_5
{
    public partial class Form3 : Form
    {
        ...
        {
            con.Open();
            string sql = "select * from ChiTietNhanVien where MaNV = " + textBox6.Text; ;  // lay het du lieu trong bang NhanVien
        ...
        {
            con.Open();
            string sql = "select * from ChiTietNhanVien ";  // lay het du lieu trong bang NhanVien
            SqlCommand cmd = new SqlCommand(sql, con); //bat dau truy van
            cmd.CommandType = CommandType.Text;
            SqlDataAdapter da = new SqlDataAdapter(cmd); //chuyen du lieu ve
            DataTable dt = new DataTable(); //tạo một kho ảo để lưu trữ dữ liệu
